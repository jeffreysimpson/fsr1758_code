...
def compute_orbit(icrs, dt=-0.1*u.Myr, n_steps=50000):
    """Change to Galactocentric frame and then integrate orbit."""
...
# Exclude due to large errors in PM
clusters = clusters[~np.in1d(clusters.name, ['E1', "Pal3", "Pal4", "Crater",
                                             "Eridanus", "NGC6121"])]
...
annotate_pos = {"lower_left": dict(horizontalalignment='right',
                ...
                "lower_right": dict(verticalalignment='top',
                                    xytext=(1.1, 1.1),
                                    textcoords='offset pixels'),
                "upper_left": dict(horizontalalignment='right',
                                   verticalalignment='bottom',
                                   xytext=(1.1, 1.1),
                                   textcoords='offset pixels'),
                "upper_right": dict(xytext=(1.1, 1.1),
                                    textcoords='offset pixels')}
...
def plot_orbits(axes, peri, apo, e, Lz, r, s=1, marker='.',
                INTEREST=False, name=None):
    """Plot the orbits in various spaces."""
    ...
    if INTEREST:
        axes[0].annotate(cluster_name_dict[name][0], (peri.value, apo.value),
                         fontsize=4, bbox=dict(boxstyle='square',
                                               fc='w', ec='none', alpha=0.3,
                                               pad=-0.2),
...
plot_orbits(axes,
            ...
            cluster_orbit.angular_momentum()[2][0, :].to(u.kpc*u.km/u.s),
            cluster_r)
...
for cluster_interest in ["Terzan5", "NGC5139", "NGC6715", "NGC6656", "NGC7089",
                         "Djorg1", "Terzan10", "IC4499", "NGC2419", "NGC3201"]:
    cluster_idx = np.in1d(clusters.name, [cluster_interest])
    plot_orbits(axes,
                cluster_peri[cluster_idx],
                cluster_apo[cluster_idx],
                cluster_e[cluster_idx],
                cluster_orbit.angular_momentum()[2][0,:].to(u.kpc*u.km/u.s)[cluster_idx],
                cluster_r[cluster_idx],
